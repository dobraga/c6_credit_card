...
class Files:
    ...
    def process(self, password: Optional[str] = None,
                mapping: Mapping = Mapping(), force=False) -> None:
        LOG.info(mapping)
        [f.process(password, mapping, force) for f in self._files]

    def summary_all(self, by: str) -> pd.DataFrame:
        dfs = []
        for file in self:
            dfs.append(file.summary(['month', by], add_total=False).data)
...
def _get_date_from_filename(file: str) -> date:
    if s := search(r'(\d{2})(.)(\d{4})', file):
        ...
        raise Exception(
            'Need two date format on file names %m-%Y or %Y-%m')
